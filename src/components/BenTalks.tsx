    ...
    <section id="talks" className="px-5 py-16 md:px-8 lg:px-24 max-w-[1280px] mx-auto w-full">
      <div className="rounded-[28px] bg-zinc-900/80 border border-white/10 p-8 md:p-14 shadow-[0_20px_50px_rgba(0,0,0,0.4)] backdrop-blur-xl space-y-12">
        ...
        <div className="space-y-3">
          ...
          <p className="max-w-2xl text-base sm:text-lg font-light text-zinc-400 leading-relaxed">
            I speak about design systems, growth, artificial intelligence, and the craft behind building better, accessible products.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 rounded-3xl border border-white/10 bg-black/50 p-6 sm:p-8 space-y-6 flex flex-col justify-between group shadow-xl">
            <div className="space-y-4">
              <div className="relative h-[240px] sm:h-[300px] w-full rounded-2xl overflow-hidden border border-white/10 bg-zinc-950">
                <Image
                  src={mainTalk.image}
                  alt={mainTalk.title}
                  fill
                ...
                <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between z-10">
                  ...
                  <span className="text-xs font-mono text-white/90 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/10">
                    {mainTalk.stats}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <h3 className="text-2xl sm:text-3xl font-bold text-white group-hover:text-blue-400 transition-colors leading-tight">
                  {mainTalk.title}
                </h3>
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            {secondaryTalks.map((talk, idx) => (
              <div
                key={idx}
                className="flex-1 rounded-3xl border border-white/10 bg-black/40 p-6 space-y-4 hover:border-white/20 transition-all flex flex-col justify-between group"
              >
                <div className="space-y-3">
                  ...
                  <h4 className="text-lg sm:text-xl font-bold text-white group-hover:text-blue-400 transition-colors leading-snug">
                    {talk.title}
                  </h4>
                ...
                <div className="flex flex-wrap gap-2 pt-3 border-t border-white/10">
                  {talk.tags.map((tag, tIdx) => (
                    <span
                      key={tIdx}
                      className="text-[11px] font-mono bg-white/5 border border-white/10 text-zinc-400 px-2.5 py-0.5 rounded-full"
                    >
                      {tag}
                    </span>
                  ))}
